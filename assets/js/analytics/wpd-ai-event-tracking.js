...
(function($) {
	...
	var WpdAiEventTracking = {
		 ...
		 */
		init: function() {
			if (typeof window.WpdAiClient !== 'undefined' && typeof window.WpdAiClient.init === 'function') {
				window.WpdAiClient.init();
			}
		...
		initEngagedSessionTracking: function() {
			...
			var existingCookie = (typeof window.WpdAiClient !== 'undefined' && window.WpdAiClient.isEngagedSession())
				? '1'
				: this.getCookie(this.config.engagedSessionCookie);
			if (existingCookie) {
		...
		markSessionAsEngaged: function() {
			...
			if (typeof window.WpdAiClient !== 'undefined' && typeof window.WpdAiClient.markEngaged === 'function') {
				window.WpdAiClient.markEngaged();
			} else {
			...
			var cookieCheck = (typeof window.WpdAiClient !== 'undefined' && window.WpdAiClient.isEngagedSession())
				? '1'
				: this.getCookie(this.config.engagedSessionCookie);
		...
		buildPayload: function(data) {
			...
			if (typeof window.WpdAiClient !== 'undefined' && typeof window.WpdAiClient.getTrackingContext === 'function') {
				var trackingContext = window.WpdAiClient.getTrackingContext();
				payload.session_id = trackingContext.session_id;
				payload.landing_page = trackingContext.landing_page;
				payload.referral_source = trackingContext.referral_source;
				if (!payload.page_href) {
					payload.page_href = trackingContext.page_href;
				}
			}
