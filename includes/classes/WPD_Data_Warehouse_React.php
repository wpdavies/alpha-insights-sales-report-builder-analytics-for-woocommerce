    ...
    public function fetch_subscriptions_data() {
        ...
        foreach( $order_data_table as $order_id => $order_data ) {
			...
			if ( wpd_is_memory_usage_greater_than(90) ) {
				$this->set_error( 
					sprintf( __( 'You\'ve exhausted your memory usage after %s out of %s orders. Increase your PHP memory limit or reduce the date range. Your current PHP memory limit is %s.', WPD_AI_TEXT_DOMAIN ), $subscription_id, $total_db_records, $memory_limit ) 
				);
				break;
			}
    ...
    public function fetch_sales_data() {
        ...
        while( $offset < $total_batches ) {
            ...
            foreach ( $current_order_ids_batch as $order_id ) {
                ...
                if ( wpd_is_memory_usage_greater_than(90) ) {
                    $this->set_error( 
                        sprintf( __( 'You\'ve exhausted your memory usage after %s out of %s orders. Increase your PHP memory limit or reduce the date range. Your current PHP memory limit is %s.', WPD_AI_TEXT_DOMAIN ), $total_order_count, $total_db_records, $memory_limit ) 
                    );
                    break;
                }
    ...
    public function fetch_expense_data() {
    	...
    	foreach ( $expense_post_ids as $expense_post_id ) {
			...
			if ( wpd_is_memory_usage_greater_than(90) ) {
				...
				$memory_limit = ini_get('memory_limit');
				$this->set_error( 
					sprintf( __( 'You\'ve exhausted your memory usage. Increase your PHP memory limit or reduce the date range. Your current PHP memory limit is %s.', WPD_AI_TEXT_DOMAIN ), $memory_limit ) 
				);
