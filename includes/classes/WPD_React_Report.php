...
class WPD_React_Report {
    ...
    public static function ajax_export_all_data_csv() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( [
                'message' => __( 'You do not have permission to export data', WPD_AI_TEXT_DOMAIN )
            ] );
            return;
        }
        ...
        if ( empty( $dashboard_id ) ) {
            wp_send_json_error( [
                'message' => __( 'Dashboard ID is required', WPD_AI_TEXT_DOMAIN )
            ] );
            return;
        }
        ...
        if ( empty( $dashboard_config ) ) {
            wp_send_json_error( [
                'message' => __( 'Dashboard configuration not found', WPD_AI_TEXT_DOMAIN )
            ] );
            return;
        }
        ...
        if ( ! isset( $data_response['data'] ) || empty( $data_response['data'] ) ) {
            wp_send_json_error( [
                'message' => __( 'No data available to export', WPD_AI_TEXT_DOMAIN )
            ] );
            return;
        }
    ...
    public static function get_live_share_links_ajax_handler() {
        ...
        $nonce_valid = wp_verify_nonce($_POST['nonce'] ?? '', WPD_AI_AJAX_NONCE_ACTION) || 
                       wp_verify_nonce($_POST['nonce'] ?? '', 'wpd_live_share_nonce');
        
        if (!$nonce_valid) {
            wp_send_json_error( array(
                'message' => __('Invalid nonce', WPD_AI_TEXT_DOMAIN)
            ) );
            return;
        }
        ...
        if (empty($report_slug)) {
            wp_send_json_error( array(
                'message' => __('Report slug is required', WPD_AI_TEXT_DOMAIN)
            ) );
            return;
        }
    ...
    public static function create_live_share_link_ajax_handler() {
        ...
        if (empty($report_slug) || empty($name)) {
            wp_send_json_error( array(
                'message' => __('Report slug and name are required', WPD_AI_TEXT_DOMAIN)
            ) );
            return;
        }
    ...
    public static function delete_live_share_link_ajax_handler() {
        ...
        if (empty($link_id)) {
            wp_send_json_error( array(
                'message' => __('Link ID is required', WPD_AI_TEXT_DOMAIN)
            ) );
            return;
        }
    ...
    public static function import_json_report_ajax_handler() {
        ...
        if (empty($report_data_json)) {
            wp_send_json_error( array(
                'message' => __( 'No report data provided.', WPD_AI_TEXT_DOMAIN )
            ) );
            return;
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            wp_send_json_error( array(
                'message' => __( 'Invalid JSON format.', WPD_AI_TEXT_DOMAIN )
            ) );
            return;
        }
        ...
        if (empty($report_data['dashboard_id'])) {
            wp_send_json_error( array(
                'message' => __( 'Report dashboard_id is required.', WPD_AI_TEXT_DOMAIN )
            ) );
            return;
        }
    ...
    public static function save_report_config_ajax_handler() {
        // Verify nonce
        if ( ! wp_verify_nonce( $_POST['nonce'] ?? '', WPD_AI_AJAX_NONCE_ACTION ) ) {
            wp_send_json_error( array( 'message' => __( 'Security check failed', WPD_AI_TEXT_DOMAIN ) ) );
            return;
        }
        ...
        if (!empty($config_json)) {
            ...
            if ($json_error !== JSON_ERROR_NONE) {
                wp_send_json_error( array( 
                    'message' => sprintf( __( 'Invalid JSON configuration data: %s', WPD_AI_TEXT_DOMAIN ), esc_html( json_last_error_msg() ) )
                ) );
                return;
            }
        }
    ...
    public static function get_live_dashboard_data_ajax_handler() {
        ...
        $regular_nonce_valid = wp_verify_nonce($_POST['nonce'], WPD_AI_AJAX_NONCE_ACTION);
        $live_share_nonce_valid = wp_verify_nonce($_POST['nonce'], 'wpd_live_share_nonce');
    ...
    public static function create_report_ajax_handler() {
        ...
        foreach ($required_fields as $field) {
            if (empty($_POST[$field])) {
                wp_send_json_error( array(
                    'message' => sprintf(__('Missing required field: %s', WPD_AI_TEXT_DOMAIN), esc_html($field))
                ) );
                return;
            }
        }
    ...
    public static function delete_report_ajax_handler() {
        ...
        if (empty($_POST['report_id'])) {
            wp_send_json_error( array(
                'message' => __('Missing report ID.', WPD_AI_TEXT_DOMAIN)
            ) );
            return;
        }
    ...
    public function mark_cache_complete() {
        ...
        $response = array(
            'success' => true,
            'message' => __( 'Cache status updated to complete.', WPD_AI_TEXT_DOMAIN ),
            'data' => array(
                'cache_is_built' => 1
            )
        );
    ...
    public function delete_report($report_id) {
        ...
        if ( in_array( $report_id, self::$mandatory_report_slugs ) ) {
            ...
            $response['message'] = __('This is a required Alpha Insights report and cannot be deleted. You can hide it from the menu by unchecking "Appear in Menu" in the report settings.', WPD_AI_TEXT_DOMAIN);
        ...
        if ($existing_config === false) {
            $response['success'] = false;
            $response['message'] = __('Report not found.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if ($deleted) {
            $response['success'] = true;
            $response['message'] = __('Report deleted successfully.', WPD_AI_TEXT_DOMAIN);
        } else {
            $response['success'] = false;
            $response['message'] = __('Failed to delete report.', WPD_AI_TEXT_DOMAIN);
        }
    ...
    public function create_report($report_slug, $report_config) {
        ...
        if (empty($report_slug) || empty($report_config)) {
            $response['success'] = false;
            $response['message'] = __('Report slug and config data are required.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if (empty($report_config['name'])) {
            $response['success'] = false;
            $response['message'] = __('Report name is required in config data.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if (!preg_match('/^[a-z0-9-]+$/', $report_slug)) {
            $response['success'] = false;
            $response['message'] = __('Report slug can only contain lowercase letters, numbers, and hyphens.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if (!preg_match('/^\d+\.\d{1,2}$/', $version_number)) {
            $response['success'] = false;
            $response['message'] = __('Version number must be in format major.minor (e.g., 1.0, 1.21, 2.05).', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if ($existing_config !== false) {
            $response['success'] = false;
            $response['message'] = __('A report with this slug already exists. Please choose a different slug.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if ($saved !== false) {
            $response['success'] = true;
            $response['message'] = __('Report created successfully!', WPD_AI_TEXT_DOMAIN);
            $response['data'] = array(
            ...
            $response['success'] = false;
            $response['message'] = __('Failed to create report. Please try again.', WPD_AI_TEXT_DOMAIN);
        }
    ...
    public function update_report($original_slug, $config_data) {
        ...
        if (empty($original_slug) || empty($config_data)) {
            $response['success'] = false;
            $response['message'] = __('Original slug and config data are required.', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if ($saved !== false) {
            $response['success'] = true;
            $response['message'] = __('Report updated successfully!', WPD_AI_TEXT_DOMAIN);
            $response['data'] = array(
                'report_slug' => $report_slug,
            ...
            $response['success'] = false;
            $response['message'] = __('Failed to update report. Please try again.', WPD_AI_TEXT_DOMAIN);
        }
    ...
    public function auto_install_mandatory_report($report_slug) {
        ...
        if ( empty( $report_slug ) ) {
            $response['success'] = false;
            $response['message'] = __( 'Report slug is required.', WPD_AI_TEXT_DOMAIN );
            return $response;
        }
        ...
        if ( ! in_array( $report_slug, self::$mandatory_report_slugs ) ) {
            $response['success'] = false;
            $response['message'] = __( 'This report is not a mandatory report.', WPD_AI_TEXT_DOMAIN );
            return $response;
        }
        ...
        try {
            ...
            if ( $saved !== false ) {
                ...
                $response['message'] = sprintf( 
                    __( 'Mandatory report "%s" has been automatically installed.', WPD_AI_TEXT_DOMAIN ),
    ...
    public function import_default_report($report_slug) {
        ...
        try {
            ...
            if ( $saved !== false ) {
                $response['success'] = true;
                $response['message'] = __( 'Report imported successfully!', WPD_AI_TEXT_DOMAIN );
                $response['data'] = array(
                    'report_slug' => $report_slug,
                    'report_name' => $report_data['name'] ?? $report_slug
                );
    ...
    public function reset_default_report($report_slug) {
        ...
        try {
            ...
            if ( $saved !== false ) {
                $response['success'] = true;
                $response['message'] = __( 'Report reset to default successfully!', WPD_AI_TEXT_DOMAIN );
                $response['data'] = array(
    ...
    public function save_report_config($report_slug, $config_data) {
        ...
        try {
            ...
            if ( $existing_config === $config_data ) {
                ...
                $response['success'] = true;
                $response['message'] = __( 'Report configuration is already up to date!', WPD_AI_TEXT_DOMAIN );
            } else {
                ...
                if ( $saved !== false ) {
                    ...
                    $response['success'] = true;
                    $response['message'] = __( 'Report configuration saved successfully!', WPD_AI_TEXT_DOMAIN );
                } else {
                    throw new Exception( 'Failed to save report configuration' );
                }
            }
    ...
    public static function import_all_default_reports( $override = true ) {
        ...
        try {
            ...
            if ( $imported_count > 0 || $skipped_count > 0 ) {
                ...
                if ( $imported_count > 0 && $skipped_count > 0 ) {
                    $response['message'] = sprintf( 
                        __( 'Successfully imported %d reports. %d reports were already installed and skipped.', WPD_AI_TEXT_DOMAIN ), 
                        $imported_count,
                        $skipped_count
                    );
                } elseif ( $imported_count > 0 ) {
                    $response['message'] = sprintf( 
                        __( 'Successfully imported %d default reports.', WPD_AI_TEXT_DOMAIN ), 
                        $imported_count 
                    );
                } else {
                    $response['message'] = sprintf( 
                        __( 'All %d reports were already installed.', WPD_AI_TEXT_DOMAIN ), 
                        $skipped_count 
                    );
                }
                ...
                $response['success'] = false;
                $response['message'] = __( 'No reports could be imported.', WPD_AI_TEXT_DOMAIN );
    ...
    public function import_json_report( $report_data, $overwrite = false ) {
        ...
        try {
            ...
            if ( $existing_report && ! $overwrite ) {
                $response['success'] = false;
                $response['message'] = sprintf( 
                    __( 'Report with slug "%s" already exists. Please enable overwrite to replace it.', WPD_AI_TEXT_DOMAIN ), 
                    $dashboard_id 
                );
                return $response;
            }
            ...
            $response['message'] = $overwrite 
                ? __( 'Report successfully imported and existing report was overwritten.', WPD_AI_TEXT_DOMAIN )
                : __( 'Report successfully imported.', WPD_AI_TEXT_DOMAIN );
            ...
            $response['success'] = false;
            $response['message'] = sprintf( __( 'Error importing report: %s', WPD_AI_TEXT_DOMAIN ), esc_html( $e->getMessage() ) );
        }
    ...
    public function get_live_share_links($report_slug) {
        ...
        if (!$report_config) {
            $response['success'] = false;
            $response['message'] = sprintf( __( 'Report not found for slug: %s', WPD_AI_TEXT_DOMAIN ), esc_html( $report_slug ) );
            return $response;
        }
    ...
    public function create_live_share_link($report_slug, $name, $expiry_date, $password, $require_password) {
        ...
        if (!$saved) {
            $response['success'] = false;
            $response['message'] = __('Failed to save live share link', WPD_AI_TEXT_DOMAIN);
            return $response;
        }

        $response['success'] = true;
        $response['message'] = __('Live share link created successfully', WPD_AI_TEXT_DOMAIN);
    ...
    public function delete_live_share_link($link_id) {
        ...
        if (!$found_report_slug) {
            $response['success'] = false;
            $response['message'] = __('Live share link not found', WPD_AI_TEXT_DOMAIN);
            return $response;
        }
        ...
        if (!$saved) {
            $response['success'] = false;
            $response['message'] = __('Failed to delete live share link', WPD_AI_TEXT_DOMAIN);
            return $response;
        }

        $response['success'] = true;
        $response['message'] = __('Live share link deleted successfully', WPD_AI_TEXT_DOMAIN);
        return $response;
    }
