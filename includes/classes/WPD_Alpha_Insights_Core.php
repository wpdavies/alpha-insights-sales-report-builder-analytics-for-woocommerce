...
class WPD_Alpha_Insights_Core {
	...
	public function update_user_meta_at_registration( $user_id, $userdata ) {
		...
		// Current URL
		update_user_meta( $user_id, '_wpd_ai_registration_url_current', wpd_get_current_url_raw() );

		// Referral URL
		update_user_meta( $user_id, '_wpd_ai_registration_url_referral', wpd_get_referral_url_raw() );
	...
	public function display_user_summary_in_wp_admin( $user ) {
		...
		// Cached data
		$user_data							= wpd_fetch_customer_analytics_by_user_id( $user->ID );
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="2">
						...
						<p><strong>Lifetime Value:</strong> <?php echo wc_price( $lifetime_value ); ?></p>
						<p><strong>Average Order Value:</strong> <?php echo wc_price( $average_order_value ); ?></p>
	...
	function display_admin_order_item_meta( $item_id, $item, $product ) {
		...
		$default_line_item_cogs 	= wpd_get_cost_price_by_product_id( $product_id );
		...
		if ( is_array($custom_product_costs) && ! empty($custom_product_costs) ) {
			...
			foreach( $custom_product_costs as $custom_cost_slug => $custom_cost_data ) {
				...
				// Calculated value
				$default_calculated_value = (float) wpd_calculate_custom_product_cost_by_line_item( $item, $custom_cost_data );
	...
	public function add_profit_to_wc_dashboard_widget() {
		...
		?>
			...
			<li class="wpd-status-widget-item net-profit-this-month">
				<a href="<?php echo esc_url( wpd_admin_page_url('reports-orders') . '&wpd-report-from-date=' . esc_attr( $start_date ) . '&wpd-report-to-date=' . esc_attr( $end_date ) ); ?>">
					<strong><?php echo wc_price( $store_profit['total_store_profit'] ); ?></strong>Net Profit This Month
				</a>
			</li>
			<li class="wpd-status-widget-item gross-profit-this-month">
				<a href="<?php echo esc_url( wpd_admin_page_url('reports-orders') . '&wpd-report-from-date=' . esc_attr( $start_date ) . '&wpd-report-to-date=' . esc_attr( $end_date ) ); ?>">
		...
		?>
			...
			<?php if ( $order_data['total_order_tax'] > 0 ) : ?>
				<tr class="wpd-order-meta-summary">
					...
					<td width="1%"></td>
					<td class="total"><?php echo wc_price( $order_data['total_order_tax'] ); ?></td>
				</tr>
			<?php endif; ?>
			<tr class="wpd-order-meta-summary">
			...
			<tr class="wpd-order-meta-summary">
	            ...
	            <td width="1%"></td>
	            <td class="total"><?php echo wc_price( $order_data['total_order_profit'] ); ?></td>
	        </tr>
			<tr class="wpd-order-meta-summary">
	...
	public function add_cost_price_to_admin_order_meta_line_item( $product, $item, $item_id = null ) {
		...
		if ( is_a( $item, 'WC_Order_Item_Product' ) && is_a( $product, 'WC_Product' ) ) :
			...
			$multi_currency = 0;
			$product_data 	= array();
			$order_data 	= wpd_calculate_cost_profit_by_order( $item->get_order() );
					...
					<div class="wpd-line-item-summary">
						<table class="display_meta fixed wpd-line-item-summary">
							<tbody>
								...
								<?php endif; ?>
								<tr>
									<th><?php _e( 'Profit:', 'alpha-insights-sales-report-builder-analytics-for-woocommerce' ); ?></th>
	...
	public function order_admin_metabox_dashboard( $post_or_order_object ) {
		...
		if ( $order->get_status() == 'auto-draft'  ) {
			...
			$session_count 			= wpd_get_session_count_by_ip_address( $ip_address );
			...
			if ( is_string( $billing_email ) && ! empty( $billing_email ) ) {
				...
				if ( is_array( $customer_order_data ) && ! empty( $customer_order_data ) ) {
					...
					$order_count 			= wpd_customer_order_count_by_email_address($billing_email); // Review this
					$lifetime_value			= wpd_customer_lifetime_value_by_email_address($billing_email);
					$average_order_value 	= wpd_customer_average_order_value_by_email_address($billing_email);
		...
		<?php if ( ! wpd_is_license_active() ) : ?><a href="<?php echo wpd_invalid_license_url() ?>" class="button wpd-input button-primary wpd-license-inactive-button">Please activate Alpha Insights.</a><?php endif; ?>
		<style type="text/css">
		...
		<div class="wpd-order-dashboard <?php if ( ! wpd_is_license_active() ) echo 'wpd-license-inactive'; ?> ">
				...
				<div class="edit_address"><?php
					...
					if ( is_array($order_data['product_data']) && ! empty($order_data['product_data'])  ) {
						...
						foreach( $order_data['product_data'] as $product_id_slug => $product_data ) {
							...
							if ( ! empty( wpd_get_custom_product_cost_options($product_data['product_id']) ) ) {
				...
				<table class="wpd-data-table">
					<tbody>
						...
						<tr>
							...
							<td><?php echo wpd_strip_query_parameters_from_url( $landing_page ); ?></td>
						</tr>
						<tr>
							<td colspan="2">
							...
							<td colspan="2">
								...
								<select class="wpd-input" name="wpd_ai_order_meta_campaign_id">
									...
									<?php foreach( $all_meta_campaigns as $campaign_id => $campaign_name ) : ?>
										<option value="<?php echo esc_attr( $campaign_id ); ?>" <?php echo wpd_selected_option( $campaign_id, $meta_campaign_id ); ?>><?php echo esc_html( $campaign_name ); ?></option>
									<?php endforeach; ?>
								</select>
							</td>
						</tr>
			...
			<div class="wpd-order-stat">
					<div class="wpd-order-stat-data wpd-statistic"><?php echo ucfirst($new_returning_customer); ?></div>
				...
				<div class="wpd-order-stat">
					<div class="wpd-order-stat-data wpd-statistic"><?php echo wc_price( $average_order_value ); ?></div>
	...
	public function save_update_order_details( $order_id, $order ) {
		...
		if ( is_a($order, 'WC_Order') ) {
			...
			$url = wpd_get_current_url_path_raw();
			...
			// Allow us to save or manipulate meta before it gets calculatd
	...
	public function register_admin_order_columns( $columns ) {
		...
		// Don't load HTML for non-authorized users
		if ( ! wpd_is_user_authorized_to_view_alpha_insights() ) return $columns;
	...
	public function display_admin_order_column_data( $column, $post_id_or_order_object ) {
		...
		if ( 'order_profit' == $column || 'order_margin' == $column || 'wpd_ai_new_vs_returning' == $column ) {
			...
			// Data
	        $order_datas 			= wpd_calculate_cost_profit_by_order( $order_id );
		...
		if ( $column == 'wpd_ai_traffic_source' ) {
			...
			echo '<mark class="wpd-order-acquisition-channel order-status tips '.strtolower( $traffic_type ).'" data-tip="'.$referral_source_url.'"><span>'.$traffic_type .'</span></mark>';
	...
	public function register_product_cost_of_goods_tab( $product_data_tabs ) {
		...
		// Don't load HTML for non-authorized users
		if ( ! wpd_is_user_authorized_to_view_alpha_insights() ) return $product_data_tabs;
							...
							<?php if ( ! $is_variable ) : ?>
								...
								<tr>
									<?php
										...
										$cost_price 	= (float) wpd_get_cost_price_by_product_id( $product_id );
										$additional_cost_string = '';

										// Additional Cost Strings
										if ( ! empty($custom_product_costs) ) {
											$additional_costs = wpd_get_additional_costs_by_product_id( $product_id );
											$cost_price += (float) $additional_costs['total'];
										}

										// Calculate Margin
										$profit = $current_price - $cost_price;
										$margin	= wpd_calculate_margin($profit, $current_price);

									?>
									<td><?php echo wc_price( $current_price ); ?></td>
									<td><?php echo wc_price( $cost_price ); ?></td>
								...
								<tbody>
									<?php foreach ( $variation_ids as $variation_id ) : ?>
										...
										<tr>
											...
											<td>
												<?php $profit = (float) $variation->get_price() - (float) wpd_get_cost_price_by_product_id($variation_id); ?>
	...
	public function save_product_cog_data( $product_id, $__post ) {
		...
		$update = wpd_update_product_cache_by_product_id( $product_id );

		// Clear cache for orders containing the updated products
		if ( ! empty( $updated_product_ids ) && is_array($updated_product_ids) ) {
			wpd_delete_order_cache_by_product_ids( array_unique( $updated_product_ids ) );
		}
	...
	public function print_product_custom_admin_columns( $column, $post_id ) {
		...
		if ( $column === 'wpd_ai_cost_of_goods' ) {
			...
			if ( $has_children) {
				...
				$cogs_range = wpd_get_min_max_product_cogs_range( $product );
				if ( $cogs_range ) {
				...
				// Simple product
				echo wc_price( wpd_get_cost_price_by_product_id( $post_id ) );
			...
			if ( $has_children) {
				...
				$margin_range = wpd_get_min_max_product_margin_range( $product );
				...
				$margin_percentage = wpd_calculate_margin_by_product( $product );
			...
			$product_analytics 	= wpd_fetch_product_analytics_by_product_id( $post_id );
	...
	public function output_user_custom_columns( $value, $column_name, $user_id ) {
		...
		if ( $column_name === 'wpd_ai_sessions' ) {
			...
			$customer_analytics = wpd_fetch_customer_analytics_by_user_id( $user_id);
