	...
	function wpd_generate_pdf_from_report_slug( $report_slug ) {
		...
		try {
			...
			if ( empty( $report_slug ) ) {
				throw new Exception( __('Report slug is required', WPD_AI_TEXT_DOMAIN) );
			}

			// Validate license key
			if ( empty( $license_key ) ) {
				throw new Exception( __('You must have a valid license key to use this feature', WPD_AI_TEXT_DOMAIN) );
			}
			...
			if ( ! $report_config ) {
				throw new Exception( __('Report not found', WPD_AI_TEXT_DOMAIN) );
			}
			...
			if ( ! isset( $link_creation_result['success'] ) || ! $link_creation_result['success'] ) {
				throw new Exception( 
					isset( $link_creation_result['message'] ) 
						...
						: __('Failed to create temporary live link', WPD_AI_TEXT_DOMAIN) 
				);
			}
			...
			if ( is_wp_error( $api_response ) ) {
				throw new Exception( 
					__('API request failed: ', WPD_AI_TEXT_DOMAIN) . $api_response->get_error_message() 
				);
			}
			...
			if ( $response_code !== 200 ) {
				...
				$error_msg = isset( $error_data['error_message'] ) 
					...
					: __('PDF generation API returned error', WPD_AI_TEXT_DOMAIN);
			...
			if ( ! isset( $pdf_data['success'] ) || ! $pdf_data['success'] ) {
				throw new Exception( 
					isset( $pdf_data['error_message'] ) 
						...
						: __('PDF generation failed', WPD_AI_TEXT_DOMAIN) 
				);
			}

			if ( empty( $pdf_data['pdf_base64'] ) ) {
				throw new Exception( __('PDF data is missing from API response', WPD_AI_TEXT_DOMAIN) );
			}
			...
			if ( $pdf_binary === false ) {
				throw new Exception( __('Failed to decode PDF data', WPD_AI_TEXT_DOMAIN) );
			}
			...
			if ( $bytes_written === false ) {
				throw new Exception( __('Failed to save PDF file to server', WPD_AI_TEXT_DOMAIN) );
			}
			...
			if ( ! empty( $temp_link_id ) ) {
				try {
					...
					if ( $success ) {
						...
						$response['cleanup_warning'] = __('Temporary live link could not be deleted', WPD_AI_TEXT_DOMAIN);
					}
				}
			}
