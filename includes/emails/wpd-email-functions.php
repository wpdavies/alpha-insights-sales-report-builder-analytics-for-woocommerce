...
function wpd_email( $email, $preview = false, $args = array() ) {
	...
	if ( $email === 'wpd_profit_report' ) {
		...
		try {
			...
			( ! empty( $args['subject'] ) ) ? $mail_subject = $args['subject'] : $mail_subject = __( $site_name . ' Profit Report', 'wpd-alpha-insights' );
			...
			$mail_subject = __( $site_name . ' Profit Report - Error', 'wpd-alpha-insights' );
		}
	...
	elseif ( $email === 'wpd_expense_report' ) {
		...
		try {
			...
			( ! empty( $args['subject'] ) ) ? $mail_subject = $args['subject'] : $mail_subject 	= __( $site_name . ' Expense Report', 'wpd-alpha-insights' );
			...
			$mail_to = $admin_email;
			$mail_subject = __( $site_name . ' Expense Report - Error', 'wpd-alpha-insights' );
		}
	 ...
	 */
		...
		$response['message'] = __( 'Email doesn\'t exist.', 'wpd-alpha-insights' );
	...
	if ( $mail_send === true ) {
		...
		$response['email_sent'] = false;
		$response['message'] = __( 'couldn\'t send email', 'wpd-alpha-insights' );
...
function wpd_email_header( $heading, $subheading = null ) {
	...
	if ( $subheading === null ) {
		...
		$subheading = __( 'Created for ', 'wpd-alpha-insights' ) . ' ' . $site_name;
