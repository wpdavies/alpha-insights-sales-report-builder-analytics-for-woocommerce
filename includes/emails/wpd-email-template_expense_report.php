...
?>
<?php wpd_email_header( sprintf( __( '%s Expense Report', 'wpd-alpha-insights' ), 'Alpha Insights' ) ); ?>
					                ...
					                <table border="0" cellpadding="20" cellspacing="0" width="100%" id="emailContainer">
					                    <tbody>
					                    	<?php if ( $expense_report_settings['details']['total_expenses_paid'] ) : ?>
							                    <?php wpd_table_row_report_data( __( 'Total Expenses Paid', 'wpd-alpha-insights' ), wc_price($expense_report_totals['total_amount_paid']) ); ?>
						                	<?php endif; ?>
						               		<?php if ( $expense_report_settings['details']['total_no_expenses'] ) : ?>
							                    <?php wpd_table_row_report_data( __( 'Total No. Expenses', 'wpd-alpha-insights' ), $expense_report_totals['total_expense_count'] ); ?>
						                	<?php endif; ?>
						               		<?php if ( $expense_report_settings['details']['average_expenses_per_day'] ) : ?>
							                    <?php wpd_table_row_report_data( __( 'Average Expenses Per Day', 'wpd-alpha-insights' ), wc_price($expense_report_totals['average_expenses_per_day']) ); ?>
						                	<?php endif; ?>
						               		<?php if ( $expense_report_settings['details']['parent_expenses'] ) : ?>
						               			<?php foreach( $expense_report_categorized['parent_expense_type_categories'] as $key => $value ) : ?>
							                    	<?php wpd_table_row_report_data( sprintf( __( 'Spent On %s', 'wpd-alpha-insights' ), wpd_clean_string($key) ), wc_price( $value['total_amount'] ) ); ?>
							                	<?php endforeach; ?>
						                	<?php endif; ?>
